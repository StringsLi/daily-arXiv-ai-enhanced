...
const AI_ANALYSIS_FIELDS = [
  ...
  { key: 'motivation', jsonKey: 'motivation', label: '研究动机' },
  { key: 'method', jsonKey: 'method', label: '主要方法' },
  { key: 'experiments', jsonKey: 'experiments', label: '实验 / 验证' },
  { key: 'result', jsonKey: 'result', label: '主要结果' },
  { key: 'conclusion', jsonKey: 'conclusion', label: '结论' },
  { key: 'limitations', jsonKey: 'limitations', label: '局限 / 后续问题' }
];

function highlightIfNeeded(value, terms) {
  if (!value) return '';
  return terms.length > 0 ? highlightMatches(value, terms, 'keyword-highlight') : value;
}

function getAnalysisSections(paper, terms) {
  return AI_ANALYSIS_FIELDS
    .map(field => [field.label, highlightIfNeeded(paper[field.key], terms)])
    .filter(([, value]) => value && String(value).trim().length > 0);
}

function getPaperTargetUrls(paper) {
  const absUrl = paper.url || '';
  const isArxivAbs = absUrl.includes('/abs/');
  return {
    abs: absUrl,
    pdf: firstText(paper.pdfUrl, isArxivAbs ? absUrl.replace('/abs/', '/pdf/') : ''),
    html: firstText(paper.htmlUrl, isArxivAbs ? absUrl.replace('/abs/', '/html/') : '')
  };
}
  ...
  const jsonData = {
    ...
    papers: papers.map(p => {
      ...
      return {
        ...
        code_last_update: p.code_last_update,
        date: p.date,
        url: urls.abs,
        pdf_url: urls.pdf,
        html_url: urls.html,
  ...
  lines.forEach(line => {
    try {
      ...
      const paperUrl = firstText(
        ...
        paper.id ? `https://arxiv.org/abs/${paper.id}` : ''
      );
      const pdfUrl = firstText(
        paper.pdf,
        paperUrl.includes('/abs/') ? paperUrl.replace('/abs/', '/pdf/') : ''
      );
      const htmlUrl = paperUrl.includes('/abs/') ? paperUrl.replace('/abs/', '/html/') : '';
...
function renderPapers() {
  ...
  filteredPapers.forEach((paper, index) => {
    ...
    const cardInsightKeys = ['keyInnovation', 'method', 'conclusion'];
    const cardInsights = AI_ANALYSIS_FIELDS
      .filter(field => cardInsightKeys.includes(field.key))
      .map(field => [field.label, highlightIfNeeded(paper[field.key], titleSummaryTerms)])
      .filter(([, value]) => value && String(value).trim().length > 0);
    ...
    paperCard.innerHTML = `
      ...
      ${paper.isMatched ? '<div class="match-badge" title="匹配你的筛选条件"></div>' : ''}
...
function showPaperDetails(paper, paperIndex) {
  ...
  const urls = getPaperTargetUrls(paper);
  ...
  const highlightedTitle = highlightIfNeeded(paper.title, modalTitleTerms);
  ...
  const displayAbstract = firstText(paper.details, paper.originalSummary);
  const abstractTitle = paper.details ? '中文摘要' : 'Original Abstract';
  const highlightedAbstract = highlightIfNeeded(displayAbstract, modalTitleTerms);
  const highlightedOriginalAbstract = paper.details ? highlightIfNeeded(paper.originalSummary || '', modalTitleTerms) : '';
  const detailSectionKeys = ['keyInnovation', 'motivation', 'method', 'result', 'conclusion'];
  ...
  const paperInfoRows = [
    ['作者', highlightedAuthors],
    ['分类', categoryDisplay],
    ['来源', firstText(paper.source, paper.sourceType)],
    ['期刊', paper.journal],
    ['DOI', paper.doi ? `<a href="https://doi.org/${paper.doi}" target="_blank">${paper.doi}</a>` : ''],
    ['arXiv ID', firstText(paper.arxivId, paper.id)],
    ['日期', paper.date ? formatDate(paper.date) : ''],
    ['原始分类', paper.rawCategories && paper.rawCategories.length ? paper.rawCategories.join(', ') : ''],
    ['备注', paper.comment],
    ['来源链接', paper.sourceUrl ? `<a href="${paper.sourceUrl}" target="_blank">${paper.sourceUrl}</a>` : ''],
    ['代码', paper.code_url ? `<a href="${paper.code_url}" target="_blank">${paper.code_url}</a>${paper.code_stars ? ` · ${paper.code_stars} stars` : ''}${paper.code_last_update ? ` · 更新 ${paper.code_last_update}` : ''}` : '']
  ].filter(([, value]) => value && String(value).trim().length > 0);

  const paperInfoHtml = paperInfoRows.map(([label, value]) => `
  ...
  const pdfPreviewHtml = urls.pdf
    ? `<div class="pdf-preview-section">
        ...
        <div class="pdf-container">
          <iframe src="${urls.pdf}" width="100%" height="800px" frameborder="0"></iframe>
        </div>
      </div>`
    : '';
  ...
  const modalContent = `
    <div class="paper-details ${matchedPaperClass}">
      ...
      ${highlightedAbstract ? `<section class="paper-detail-summary"><h3>${abstractTitle}</h3><p class="translated-abstract">${highlightedAbstract}</p></section>` : ''}
  ...
  pdfLink.style.display = urls.pdf ? 'flex' : 'none';
  htmlLink.href = urls.html || '#';
  htmlLink.style.display = urls.html ? 'flex' : 'none';
  ...
  const prompt = `请你阅读这篇论文 ${urls.pdf || urls.abs}，总结这篇文章解决的问题、相关工作、研究方法、实验与结果、结论，并整体概括论文内容。`;
  const systemPrompt = '你是一个学术助手。后续对话将围绕这篇论文展开，请用专业、准确、结构化的中文回答。';
