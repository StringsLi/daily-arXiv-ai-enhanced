...
免费全文: [arXiv abstract](https://arxiv.org/abs/2609.04177v1), [arXiv pdf](https://arxiv.org/pdf/2609.04177v1), [Google Scholar search](https://scholar.google.com/scholar?q=A+Computationally+Feasible+Framework+for+Causal+Probabilistic+Explanation++free+full+text+pdf)

一句话结论: 提出PCI方法，通过蒙特卡洛近似解决因果解释问题。

研究问题: 现有因果解释工具存在两难困境：实际因果关系（AC）理论提供原则性结论但仅适用于玩具模型，而可扩展的归因方法如SHAP忽略了因果结构，可能给出与因果分析相冲突的结果。

主要创新点: 提出概率因果影响（PCI）方法，将因果可解释性问题重新构建为概率因果模型上的蒙特卡洛估计问题，提供可处理、基于因果的分级解释，推广了实际因果关系和Pearl的因果概率概念。


<details>
  <summary>更多信息</summary>

研究动机: 为了解决因果解释工具的两难困境：AC理论提供原则性结论但计算成本高、仅适用于小规模模型，而可扩展的归因方法忽略了因果结构，可能给出错误解释。

主要方法: PCI基于实际因果关系和Pearl的必要性和充分性概率概念，通过指定候选解释分布、反事实值分布和评分函数，将因果可解释性问题重新构建为可轻易通过蒙特卡洛近似估计的概率因果模型上的估计问题，提供可处理的因果分级解释。

实验 / 验证: 在合成和真实世界例子中评估PCI，包括与AC的一致性检查、扩展性实验、复杂连续值动态系统，以及在数百万数据点上训练的真实世界部署因果机器学习模型。

主要结果: PCI在多种场景下验证了其有效性和可行性，能够提供与因果分析一致的解释，同时适用于大规模真实世界数据。

结论: PCI成功填补了因果解释工具的空白，提供了一种可处理、基于因果的分级解释方法，既推广了实际因果关系理论，又解决了可扩展性问题。

局限 / 后续问题: 

中文摘要: 解释特定结果为何发生，以及哪些输入应受到责备或赞誉，是哲学、科学和政策分析的核心。现有工具分为两大阵营。实际因果关系（AC）理论给出了原则性结论，但由于计算需要枚举反事实情景，仅适用于玩具大小的模型。可扩展的归因方法如SHAP（甚至因果SHAP）至少部分忽略了生成数据的因果结构，可能给出与细致因果分析相冲突的答案。我们通过概率因果影响（PCI）填补这一差距。PCI基于实际因果关系和Pearl的必要性和充分性概率概念，但将可解释性问题重新构建为可轻易通过蒙特卡洛近似估计的概率因果模型上的估计问题。通过指定“候选解释”分布、反事实值分布和评分函数，PCI提供可处理、基于因果的分级解释，推广了AC和Pearl的因果概率作为退化情况。我们在合成和真实世界的例子中评估PCI，包括与AC的一致性检查、扩展性实验、复杂连续值动态系统，以及在数百万数据点上训练的真实世界部署因果机器学习模型。

英文原始摘要: Explaining why a specific outcome occurred, and which inputs deserve the blame or credit, is central to philosophical, scientific, and policy analysis. Existing tools split into two camps. The theory of actual causality (AC) gives principled verdicts, but only for toy-sized models, because computing them requires enumerating counterfactual scenarios. Scalable attribution methods like SHAP (or even causal SHAP) at least partially ignore the causal structure that generated the data, and can give answers that conflict with a careful causal analysis. We close this gap with Probabilistic Causal Impact (PCI).
...
### [2] [Non-Kolmogorov-Arnold-Moser Quantum Sensors for Quantum Parameter Estimation](https://arxiv.org/abs/2609.03092v1)
...
免费全文: [arXiv abstract](https://arxiv.org/abs/2608.27521v1), [arXiv pdf](https://arxiv.org/pdf/2608.27521v1), [Google Scholar search](https://scholar.google.com/scholar?q=Dandelion%3A+A+Spherical+Flower+for+Neural+Simulation+of+Planetary+Dynamics++free+full+text+pdf)

一句话结论: Dandelion是球面warp神经PDE求解器，通过球面坐标变换实现无卷积的空间混合，在球面PDE基准测试中表现.

研究问题: 欧几里得机器学习架构应用于球面网格时存在固有缺陷，包括高纬度处笛卡尔卷积变形、2D FFT错误假设双重周期性、ViT中位置编码扭曲测地距离等问题，且现有球面数据集要么太小且风格化，要么过大（如ERA5）以至于无法进行模型迭代。

主要创新点: 提出Dandelion球面warp神经PDE求解器，通过在球谐域实现分层池化实现无卷积结构，仅通过球面坐标变换进行空间混合，并发布了一套具有挑战性的原生平面PDE基准数据集。


<details>
  <summary>更多信息</summary>

研究动机: 球面上许多动力过程需要专门的球面机器学习架构，但现有方案（如球面卷积、SFNOs）仍有局限，且缺乏合适的基准数据集。

主要方法: Dandelion是Flower的球面版本，预测切平面位移并沿大圆传输特征，通过在球谐域实现分层池化构建U-Net结构，仅使用球面坐标变换而非卷积进行空间混合。

实验 / 验证: 发布了不断演进的基准套件，包含6个具有挑战性的原生平面PDE数据集：修改版Galewsky急流、异常链式湍流、Cahn-Hilliard分解、球面黎曼激波、Held-Suarez干大气传输和全球海洋动力学。

主要结果: Dandelion在每个数据集上排名最佳或第二，且与非warp基线的差距随分辨率增大，在256×512分辨率下单步预测和滚动预测中均占据前两名。

结论: Dandelion证明了球面warp架构的有效性，填补了球面基准数据集的空白，在多个球面PDE问题上表现出色。

局限 / 后续问题: 

中文摘要: 许多动力过程发生在球面上，但默认的机器学习架构是欧几里得的。将这些架构应用于常规经纬度网格会导致问题：笛卡尔卷积在高纬度处变形；傅里叶神经算子中的2D FFT错误地假设了双重周期性；ViT中的笛卡尔位置编码扭曲了球面测地距离。最近的工作转向了原生的球面原语，包括球面卷积（如DeepSphere或DISCO）、球面傅里叶神经算子（SFNOs）和测地注意力。在此，我们提出了Dandelion，即基于warp的神经PDE求解器Flower的球面版本。Dandelion的层预测切平面位移并沿大圆传输特征。通过完全在球谐域中实现分层池化，我们获得了类似U-Net的结构。因此没有卷积：空间混合仅通过球面坐标变化或warp实现。为了与现有的球面架构进行比较，我们发布了一个不断演进的基准套件，包含具有挑战性的原生平面PDE数据集，包括修改版的Galewsky急流、异常链式湍流、Cahn-Hilliard分解、球面黎曼激波、Held-Suarez干大气传输和全球海洋动力学。这个新基准填补了现有球面数据集的空白，它们要么太小且风格化，要么大得多（ERA5）以至于无法进行模型迭代。Dandelion在每个数据集上都排名最佳或第二，且与非warp基线的差距随分辨率增大：在256×512分辨率下，Dandelion和Flower2D在单步预测和滚动预测中都占据前两名。

英文原始摘要: Many dynamical processes unfold on the sphere but the default scientific machine learning architectures are Euclidean. Applying these architectures on a regular lat-lon grid causes problems: Cartesian convolutions become distorted at high latitude; 2D FFTs in Fourier neural operators incorrectly assume double periodicity; Cartesian positional encodings in ViTs distort spherical geodesic distances. Recent work moves towards natively spherical primitives, including spherical convolutions (e.g., DeepSphere or DISCO), Spherical Fourier Neural Operators (SFNOs), and geodesic attention. Here we propose Dandelion, a spherical version of Flower, a warp-based neural PDE solver. Layers of Dandelion predict a tangent-plane displacement and transport features along great circles. We obtain a U-Net-like structure by implementing hierarchical pooling entirely in the spherical-harmonic domain. There are thus no convolutions: spatial mixing is achieved only through spherical coordinate changes, or warps. To compare Dandelion with existing spherical architectures, we release an evolving benchmark suite of challenging, natively-spherical PDE datasets including a modified Galewsky jet, anomalous chained turbulence, Cahn-Hilliard decomposition, spherical Riemann shocks, Held-Suarez dry atmospheric transport and global ocean dynamics. This new benchmark fills the gap in existing spherical datasets which are either too small and stylized, or much too large (ERA5) for model iteration. Dandelion is best or second-best on every dataset, and the gap to non-warp baselines widens with resolution: at $256\times 512$, Dandelion and Flower2D occupy the top two slots in both single-step prediction and rollout.

</details>


### [5] [Latent-Frequency Validity: Fast Spectral Editing with Screened Video-VAE Transfer Operators](https://arxiv.org/abs/2608.07569v1)
...
免费全文: [arXiv abstract](https://arxiv.org/abs/2608.07569v1), [arXiv pdf](https://arxiv.org/pdf/2608.07569v1), [Google Scholar search](https://scholar.google.com/scholar?q=Latent-Frequency+Validity%3A+Fast+Spectral+Editing+with+Screened+Video-VAE+Transfer+Operators++free+full+text+pdf)

一句话结论: LFV通过验证选择路径控制跨通道容量，实现高效且高质量的潜空间频谱编辑

研究问题: 直接在视频VAE潜空间中进行频谱编辑时，VAE可能会在潜空间通道之间重新分配像素空间的频带，导致潜空间编辑破坏VAE往返动力学

主要创新点: 提出了潜空间频率有效性（LFV），通过学习紧凑的VAE特定频谱响应和验证选择路径，使跨通道容量成为可编辑资源的可控属性


<details>
  <summary>更多信息</summary>

研究动机: 需要在不破坏VAE往返动力学的情况下，高效地在视频VAE潜空间中实现频谱编辑

主要方法: LFV学习一个紧凑的VAE特定频谱响应，遵循从对频校准器（C1）到完全通道混合（CM）的验证选择路径，仅在提高解码目标保真度且不恶化往返漂移时部署

实验 / 验证: 在544个VAE编辑单元（跨越六个频谱族）上测试，包括主要的120单元径向扫描和五个额外的滤波器族，使用源视频分组验证评估；还测试了完全冻结的OpenVid拟合算子在CogVideoX和HunyuanVideo生成域单元上的表现

主要结果: LFV在544个单元上生成了423个廉价算子，99/100个在主要扫描中通过验证，所有323个在额外滤波器族上通过验证，完全冻结的算子无需调整即可通过所有20个测试单元；选择的响应比像素滤波-重新编码快约3倍

结论: LFV提供了有效的潜空间频谱编辑方法，在保持往返动力学稳定的同时实现了快速和高质量的编辑

局限 / 后续问题: 

中文摘要: 直接在视频VAE潜空间中进行频谱编辑可以控制噪声、闪烁、平滑度和频率内容，而不需要解码-过滤-重新编码的流程。然而，视频VAE可能会在潜空间通道之间重新分配像素空间的频带，而潜空间编辑可能会破坏VAE往返动力学。本文引入了“潜空间频率有效性”（LFV），学习一个紧凑的VAE特定频谱响应，仅当它提高解码目标保真度且不恶化往返漂移时才部署LFV。LFV遵循一条从对频校准器（C1）到完全通道混合（CM）的验证选择路径，使跨通道容量成为可编辑资源的可控属性。在跨越六个频谱族的544个VAE编辑单元上，LFV生成了423个廉价算子：277个由C1处理，而146个（占生成算子的34.5%）需要通道混合。在主要的120单元径向扫描中，99/100个生成的算子通过源视频分组验证评估。在五个额外的滤波器族上，所有323个生成的算子都通过验证评估。完全冻结的OpenVid拟合算子，包括验证选择路径系数，在所有20个测试的CogVideoX和HunyuanVideo生成域单元上通过，无需调整。选择的响应匹配直接潜空间滤波延迟，比像素滤波-重新编码快约3倍。结果图揭示了不同的VAE状态，包括强通道耦合的CogVideoX响应和尖锐的Open-Sora高带稳定性前沿。

英文原始摘要: Direct spectral editing in video-VAE latents can control noise, flicker, smoothness, and frequency content without a decode--filter--reencode pass. However, video VAEs may redistribute pixel-space frequency bands across latent channels, and latent edits can disrupt VAE round-trip dynamics. We introduce \emph{latent-frequency validity} (LFV), which learns a compact VAE-specific spectral response and deploys it only when it improves decoded-target fidelity without worsening round-trip drift. LFV follows a validation-selected path from a diagonal per-frequency calibrator (C1) to full channel mixing (CM), making cross-channel capacity a controllable per-edit resource. Across 544 VAE--edit cells spanning six spectral families, LFV emits 423 cheap operators: 277 are handled by C1, while 146 (34.5\% of emitted operators) require channel mixing. On the primary 120-cell radial sweep, 99/100 emitted operators pass source-video-grouped held-out evaluation. Across five additional filter families, all 323 emitted operators pass held-out evaluation. Fully frozen OpenVid-fitted operators, including the validation-selected path coefficient, pass all 20 tested CogVideoX and HunyuanVideo generated-domain cells without adaptation. The selected response matches direct latent-filter latency and is about $3\times$ faster than pixel filter--reencode. The resulting maps reveal distinct VAE regimes, including strongly channel-coupled CogVideoX responses and a sharp Open-Sora high-band stability frontier.

</details>


### [6] [Learning Discrete Decisions for MIPs with Constraint-Aware Diffusion](https://arxiv.org/abs/2608.13079v2)
...
免费全文: [arXiv abstract](https://arxiv.org/abs/2608.13079v2), [arXiv pdf](https://arxiv.org/pdf/2608.13079v2), [Google Scholar search](https://scholar.google.com/scholar?q=Learning+Discrete+Decisions+for+MIPs+with+Constraint-Aware+Diffusion++free+full+text+pdf)

一句话结论: CGD框架通过图扩散模型与可行性投影算子结合，实现混合整数优化问题的高效求解。

研究问题: 混合整数优化问题需要在满足复杂组合约束的同时联合确定离散和连续决策，计算上极具挑战性。

主要创新点: 提出名为约束图扩散（CGD）的学习型框架，将基于图的生成扩散模型与无训练的可行性投影算子直接集成到逆向扩散过程中，引导中间样本朝可行集移动。


<details>
  <summary>更多信息</summary>

研究动机: 混合整数优化问题计算困难，传统求解方法效率较低。

主要方法: CGD使用基于图的生成扩散模型学习问题的离散成分，通过将无训练的可行性投影算子直接嵌入逆向扩散过程，在生成过程中引导中间样本朝可行集移动；离散决策生成后，剩余优化转化为连续问题并利用现有数值方法高效求解。

实验 / 验证: 在交流最优潮流的最优输电切换和离散投资组合优化问题上评估CGD，与学习型基线和最先进的MINLP数值求解器进行比较。

主要结果: 在可行性和解质量上较学习型基线有显著改进，与最先进的MINLP数值求解器相比实现高达425倍的加速。

结论: CGD框架问题无关，可通过合适的投影算子处理广泛的混合整数优化问题，在可行性和求解速度上均优于现有方法。

局限 / 后续问题: 该方法的训练无可行性投影算子可能限制了其在某些问题域的应用。

中文摘要: 本文提出了一种新颖的学习型方法来近似求解混合整数优化问题实例。由于这些问题需要在满足复杂组合约束的同时联合确定离散和连续决策，计算上极具挑战性。所提出的方法依赖于基于图的生成扩散模型，在学习混合整数优化问题的离散成分的同时，将无训练的可行性投影算子直接集成到逆向扩散过程中，引导中间样本在整个生成过程中朝着可行集移动。一旦离散决策生成完毕，剩余的优化问题就转化为一个连续问题，可以利用现有的数值方法高效求解（相对于原始问题）。由此得到的名为约束图扩散（Constrained Graph Diffusion, CGD）的框架具有问题无关性，可以通过适当的投影算子容纳广泛的混合整数优化问题类别。我们在交流最优潮流的最优输电切换和离散投资组合优化上评估了CGD，证明了与学习型基线相比在可行性和解质量上都有显著改进，同时对于MINLP，与最先进的数值求解器相比实现了高达425倍的加速。
